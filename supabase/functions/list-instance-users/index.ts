...
serve(async (req) => {
  ...
  try {
    // Validate sync secret from header
    const syncSecret = req.headers.get("x-workspace-sync-secret");
    const expectedSecret = Deno.env.get("CLAIM_SYNC_SECRET");

    console.log("Received request to list instance users");
    console.log("Sync secret provided:", !!syncSecret);
    ...
    console.log("Fetching staff and admin users");
    ...
    console.error("Error in list-instance-users:", error);
    return new Response(
